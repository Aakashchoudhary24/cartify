...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#424874] text-white">
      ...
      {/* Product Carousel */}
      <div className={`${isMobile ? 'mt-12' : 'mt-20'} pb-5 ${isMobile ? 'px-4' : 'px-8'} overflow-x-auto`}>
        <div className={`flex ${isMobile ? 'flex-col items-center gap-6' : 'justify-center items-center gap-6'} ${isMobile ? '' : 'min-w-[900px] md:min-w-full'}`}>
          {[
            ...
            <motion.div
              key={index}
              className={`${isMobile ? 'w-4/5' : 'w-1/5'} aspect-[3/4] rounded-2xl overflow-hidden relative group shadow-lg`}
              initial={{ opacity: 0, x: (index - 2) * 50 }}
